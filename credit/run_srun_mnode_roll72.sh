#!/bin/bash
#SBATCH --job-name=corrdiff-1n-amp         # Job name
#SBATCH --partition=normal                 # GPU partition
#SBATCH --nodes=4                          # Number of nodes
#SBATCH --ntasks-per-node=1                # MPI tasks per node
#SBATCH --cpus-per-task=28                 # Number of CPU cores per task
#SBATCH --gres=gpu:8                       # Number of GPUs per node
#SBATCH --time=120:00:00                    # Runtime (hh:mm:ss)
#SBATCH --output=/nwpr/ait/p088/work/creditlog/%x_%j.log     # 佐~H併 stdout 佒~L stderr 佈° job name + job id
#SBATCH --error=/nwpr/ait/p088/work/creditlog/%x_%j.log
...
config="${SLURM_SUBMIT_DIR:-$(pwd)}/../config/wxformer_6hr_cwa_10y_roll72.yml"
